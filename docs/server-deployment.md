...
| **Facebook Sharing Debugger: 403 / “missing Open Graph”** | **The debugger UI can show 403 even when the scraper successfully fetches 200.** Meta crawls as **`facebookexternalhit`**, **`Facebot`**, **`meta-externalagent`**, **`meta-externalfetcher`**. After deploy, **`robots.txt`** allowlists each one with the universally-honored empty **`Disallow:`** form (the Google-extension **`Allow: /`** is ignored by some Meta parsers — see `makeRobotsTxt`). To distinguish a real server-side block from a Meta-side debugger artifact: tail nginx access logs while clicking **Scrape Again** — if you see `200` for the URL from a real Meta IP (`57.141.x`, `69.63.x`, `69.171.x`, `173.252.x`) then your origin is fine and the 403 is the [debugger's known false-positive behavior](https://developers.facebook.com/community/threads/), independent of actual link sharing. Also rule out any **WAF, Cloudflare Bot Fight Mode, fail2ban, or geo block** in front of nginx. Quick origin check: `curl -sSIL -A 'facebookexternalhit/1.1' 'https://yoochoog.app/'` → **200**. |
