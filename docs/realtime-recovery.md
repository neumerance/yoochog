...
- **Guest (join URL):** On connection/data-channel loss, starts a **new handshake immediately** (still capped by `RECONNECT_MAX_ATTEMPTS` in `reconnectPolicy.ts`), plus **page visibility**–driven recovery: after a **minimum hidden** duration, return-to-`visible` runs a **short health probe** (see `VISIBILITY_RESUME_HEALTH_PROBE_MS` in `reconnectPolicy.ts`); a full re-handshake runs only if the party link is unhealthy.
...
|-----------|---------|--------|
| Max reconnect attempts | 10 | After this many connection-loss events without a successful **connected** state, retries stop and the user sees an error. Each loss triggers an immediate re-handshake (no countdown). |
