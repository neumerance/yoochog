...
export const PLAYER_HELP_TIP_DEFINITIONS: PlayerHelpTipDefinition[] = [
  ...
  },
  {
    ...
    priority: 10,
    message:
      'Set VITE_SOCKET_URL (Socket.io server URL) so guests can sync the jukebox / karaoke queue from their phones.',
    test: (ctx) =>
  ...
  },
  {
    ...
    message: (ctx) => {
      ...
      return `Guests search YouTube from the join page and enqueue — up to ${per} per guest (including the song that’s playing) until one finishes.`
    },
    test: (ctx) =>
