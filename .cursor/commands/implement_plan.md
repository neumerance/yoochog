# implement_plan — Execute an approved plan for a GitHub issue (Cursor)
...
## Skills and commands (Cursor + Claude)

**Skill roots:** `.cursor/skills/<name>/SKILL.md` → `.claude/skills/<name>/SKILL.md` (first match wins).

**Command roots:** `.cursor/commands/` and `.claude/commands/` — keep behavior aligned when both exist.
...
- **`$ARGUMENT`**: GitHub issue number (`42`, `#42`), issue URL for this repo, or a **path/URL to a plan document** if the user is not issue-driven.
...
- **Branch:** a **feature** branch (not the repo default integration branch). Naming should follow [create-pull-request](../skills/create-pull-request/SKILL.md): `{github-login}/{issue-number}-{slug}` when tracking issue `#<number>`.
...
When switching features afterward, start a fresh chat or `/clear` so this plan does not bleed into unrelated work.
